...
function EnhancedTable() {
  const { users, total_user_count, from, rows_per_page, getUsers } = useUser();

  var totalCount = total_user_count;
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getUsers(rowsPerPage, rowsPerPage * newPage, false);
  };

  const handleChangeRowsPerPage = async (event) => {
    setPage(0);
    setRowsPerPage(parseInt(event.target.value, 10));
    getUsers(parseInt(event.target.value, 10), 0, false);
  };
